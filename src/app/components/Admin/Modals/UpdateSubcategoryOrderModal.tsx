...
import {
  useGetItemsByCountryCategory,
  useUpdateItem,
} from "@/app/services/client/ItemsService";
...
const UpdateSubcategoryOrderModal = ({
  closeModal,
  isOpen,
}: UpdateSubcategoryOrderModalProps) => {
  ...
  const { data } = useGetItemsByCountryCategory(
    selectedCountry,
    selectedCategory
  );
  ...
  useEffect(() => {
    if (isOpen && data && selectedCountry !== "" && selectedCategory !== "") {
      let items = data.slice().sort((a, b) => {
        if (
          a.subcategoryOrder === undefined ||
          b.subcategoryOrder === undefined
        ) {
          return 0;
        }
      ...
      const uniqueItems = Array.from(
        new Map(
          items
            ...
            .map((item) => [item.subcategory, item.subcategoryOrder])
        ).keys()
      );
  ...
  const updateSort = () => {
    ...
    let filteredItems = data.filter(
      (item) =>
        item.country === selectedCountry && item.category === selectedCategory
    );
    filteredItems = filteredItems.map((item) => {
      return {
        ...item,
        subcategoryOrder: subcategoryOrders.find(
          (so) => so.subcategory === item.subcategory
        )?.order,
      };
    });
